(ns shoecoord.actor
  "ShoeCoordActor — the ISCO-08 8156 shoemaking and related machine
  ...
  The unconditional invariant: the Shoemaking and Related Machine
  Operators Plant Scheduling Coordination Advisor can never directly
  commit an operating record the ShoeCoordGovernor refuses, and can
  never make this actor finalize a machine-operation-execution
  decision (e.g. deciding to proceed with a specific
  pressing/cutting/stitching run) or a plant-safety-clearance decision
  (e.g. declaring the plant safety cleared), and can never override a
  plant safety officer's judgment — every commit-record! call is
  gated behind `:decide`, and that decision class is a permanent hard
  block regardless of disposition path. This actor coordinates PLANT
  SCHEDULING/LOGISTICS ONLY — it never operates shoemaking machinery
  itself, and it never makes a plant-safety-clearance decision
  itself."
